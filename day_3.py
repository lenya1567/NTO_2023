...
test_public = rospy.Publisher('test_topic', Image, queue_size=1) # Топик для проверки работоспособности компьютерного зрения для распознавания возгораний
fireRange = ((0, 62, 0), (36, 255, 255)) # Цветовые границы для определения пожара
sides = [] # Ненужная переменная (пока не используем) 

publisher = rospy.Publisher("fires", MarkerArray, queue_size=1) # Топик для вывода положения пожара в визуализаторе, с помощью Ros3DJs 
markerArray = MarkerArray() # Массив элементов для визуализатора

get_telemetry = rospy.ServiceProxy('get_telemetry', srv.GetTelemetry) # Топик для телеметрии

def get_distance(x1, y1, z1, x2, y2, z2): # Расстояние между точками в 3D через пространственную теорему Пифагора
    return math.sqrt((x1 - x2) ** 2 + (y1 - y2) ** 2 + (z1 - z2) ** 2)

def navigate_wait(x=0, y=0, z=0, yaw=float('nan'), speed=0.5, frame_id='', auto_arm=False, tolerance=0.2): # Полёт в точку с ожиданием
...
fires = [] # Массив пожаров
def addFire(x, y): # Функция для добавления пожаров в массив, с учётом, что не засекли один и тот же пожар дважды 
    global fires, markerArray, publisher
    notNeed = False
    for fr in fires: # Проверка, что не было пожара в радиусе 50 см
        if get_distance(fr[0], fr[1], 0, x, y, 0) < 0.5:
            notNeed = True
            break
    if not notNeed: # Если не нашёл, то создаём объект, записываем в массив и отправляем в визуализатор
        marker = Marker()
        ...
        # Делаем запрос GET через функцию requests
        ...
        # Анализ того, что прислал сервер
        fireClass = "NO"
...
# Тут была часть кода, которая была дважды скопирована из терминала RPI и удалена во время запуска при организаторах
...
def range_callback(msg):
    ...
    x, y = T.x, T.y
    if len(data) == 0:
        data.append((dista, x, y)) # Начало движения с измерением длины стены
    last = data[-1]

    if abs(last[0] - dista) > 0.6: # Нашли угол стен
         
         print(last, last[1], last[2]) # Выводы отладочной информации
...
# Движение коптера по заданной траектории
...
for e in fires:
    ...
    if not notNeed:
        ...
        was += [(e[0], e[1])]
'''
print("Auto report:") # Вывод автоотчёта

print()
print("Fires:", len(fires))
print()

for i in range(len(fires)): # Определение класса пожара и вывод координат и класса пожара
    name = fires[i][2]
    group = "undefined"
...
print("Injured: 0") # Количество найденных пострадавших

c = 0
for i in range(len(data)): # Вывод длин стен 
    c += 1
